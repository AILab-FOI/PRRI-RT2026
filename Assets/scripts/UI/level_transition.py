...
class LevelTransition:
    ...
    def transition_to_next_level(self):
        ...
        if (self.next_level_num > self.game.level_manager.max_level) and (self.next_level_num != 99):
            self.transition_state = "to_victory"
            self.screen_transitions.start_fade_in(speed=8, callback=self._show_victory_screen)
            return True

        self.transition_state = "to_next_level"
        self.screen_transitions.start_fade_in(speed=8, callback=self._switch_level)
        return True
    ...
    def _switch_level(self):
        ...
        self.game.begin_new_game_load(self.next_level_num)
        self._reset_wave_related_state()

        if hasattr(self.game, 'game_ui'):
            self.game.game_ui.update_level(self.next_level_num)

        self.transition_state = "fade_back"
        self.screen_transitions.start_fade_out(speed=5, callback=self._finish_transition)
