...
class GameEvents:
    ...
    def process_events(self):
        ...
        for event in pg.event.get():
            ...
            if self.game.loading_screen.active:
                if (
                    self.game.loading_screen.level_number == 1 and
                    self.game.loading_screen.loading_complete and
                    event.type == pg.KEYDOWN and
                    event.key in (pg.K_RETURN, pg.K_KP_ENTER)
                ):
                    self.game.loading_screen.active = False
                    self.game.loading_screen.waiting_for_input = False

                    if self.game.loading_screen.on_complete_callback:
                        callback = self.game.loading_screen.on_complete_callback
                        self.game.loading_screen.on_complete_callback = None
                        callback()
                    continue

                if not self.game.loading_screen.auto_continue:
                    continue
            ...
            if hasattr(self.game, 'interaction'):
    ...
    def handle_weapon_switch(self, event):
        ...
        if event.y > 0:
            self.game.player.prev_weapon()
        elif event.y < 0:
            self.game.player.next_weapon()